<script>
  ...
  async function toggleLike(event) {
    event.preventDefault(); // Prevent form submission for JavaScript-enabled users
...
<article>
  ...
  >
    ...
    {#if isLiked}
      ...
      <input type="hidden" name="_method" value="DELETE"> <!-- Used to simulate DELETE -->
    {:else}
      <!-- For liking -->
      <input type="hidden" name="playlistId" value="{playlist.id}">
      <input type="hidden" name="profileId" value="121">
    {/if}
